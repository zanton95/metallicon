...
/**
 * Lev Comments: 
 * First of all - great work man! It's looking goooood! I can see you understand how to show and hide and you're using JS together with CSS to make a single page app.
 * Also good job on using 2 query params for the single songs view man!
 * Some notes / finishes:
 *  - Notice that you have 2 places where you listen on page loaded - with an event listener AND with window.onload. These two may collide in some cases and it makes it unclear
 *    for other people where your code should start running from. Please use one of them only.
 *  - There are errors in the console window (you can see them in DevTools -> console in chrome)
 *      + Don't forget that you can go to Sources and debug this and find out exactly where the error comes from.
 *  - If I open the page manually it will first load without a query parameter. This will show a blank page. This will also happen if I write the wrong view name.
 *    Handle these cases too - if an incorrect view is entered, go to the home page instead.
 *  - I've added some convention notes on the methods (Conventions === the way we usually organize our code and name variables)
 */
document.addEventListener("DOMContentLoaded", nameAndLyrics);


/**
 * Lev convention note: don't forget ';' at the end of the line.
 */
var songs = [
...
/**
 * Lev convention note - this is how a function should be organized, spacing-wise: (do this for al functions)
 * 
 * function nameOfFunction (parameters) {
 *  // code here, with 1 tab. Notice that the closing '}' has the same number of tabs (none in this case) as the 'fucntion ....' writing.
 * }
 */
